...
public class AuthenticationFilter implements Filter{
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) 
	    ...
	    // 1. CALL TRAINER'S METHOD HERE: This opens the bridge for every request
	    setCoRSHeaders(httpResponse); 

	    try {
	        executeFilterLogic(request, response, chain);
	    } catch(Exception e) {
	        logger.error("Unexpected error in the AuthenticationFilter", e);
	...
	private void executeFilterLogic(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		logger.info("AuthenticatedUser: {}, Role: {}", authenticatedUser.getUsername(), role);
		...
		if(requestURI.startsWith("/api") && role !=Role.CUSTOMER) {
			sendErrorResponse(httpResponse, HttpServletResponse.SC_FORBIDDEN, "Forbiddeb: Customer access required");
			return;
		}
