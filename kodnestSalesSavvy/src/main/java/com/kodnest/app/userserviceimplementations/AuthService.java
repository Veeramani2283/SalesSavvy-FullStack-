...
public class AuthService implements AuthServiceContract{
    ...
    public boolean validateToken(String token) {
        try {
            ...
            // Use parseClaimsJws if parseSignedClaims is giving a red underline/error
            Jwts.parserBuilder()
                .setSigningKey(SIGNING_KEY)
                .build()
                .parseClaimsJws(token); // Use this line
	...
	public void logout(User user) {
		...
		// get the jwt token associated with the user
		JWTToken token = jwtTokenRepository.findByUserId(userId);
		// if token exist delete it from the repository
		if(token != null) {
